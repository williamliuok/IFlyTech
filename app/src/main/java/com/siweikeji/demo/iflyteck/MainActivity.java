...
public class MainActivity extends AppCompatActivity {
    ...
    private String[] refusedReply = new String[]{"恩,这个问题有点难","对不起，我好像没有听懂","这个问题真的难倒我了"};
    private Random random = new Random();
    private IflytekWakeUp wakeUp;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        wakeUp = new IflytekWakeUp(this);
        wakeUp.startWakeuper();
            ...
            switch (event.eventType) {
                ...
                case AIUIConstant.EVENT_RESULT: {
                    ...
                    try {
                        ...
                        if (content.has("cnt_id")) {
                            ...
                            if ("nlp".equals(sub) && result.length() > 2) {
                                ...
                                if (!TextUtils.isEmpty(str)) {
                                    if (result.optInt("rc") == 0) {
                                        AIUIMessage writeMsg = new AIUIMessage(AIUIConstant.CMD_STOP_RECORD, 0, 0, "sample_rate=16000,data_type=audio", null);
                                        mAIUIAgent.sendMessage(writeMsg);
                                        SpeechUtil.speak(str);
                                    } else {
                                        str = refusedReply[random.nextInt(3)];
                                        AIUIMessage writeMsg = new AIUIMessage(AIUIConstant.CMD_STOP_RECORD, 0, 0, "sample_rate=16000,data_type=audio", null);
                                        mAIUIAgent.sendMessage(writeMsg);
                                        SpeechUtil.speak(str);
                                    }
                        ...
                        e.printStackTrace();
                        mNlpText.setText(e.getLocalizedMessage());
                    }
                ...
                case AIUIConstant.EVENT_ERROR: {
                    //错误事件
                    Log.i(TAG, "on event: " + event.eventType);
                    mNlpText.setText("错误: " + event.arg1 + "\n" + event.info);
                }
                break;
